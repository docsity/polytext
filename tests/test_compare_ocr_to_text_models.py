...
DEFAULT_OCR_PATHS = [
    "/Users/andreasolfanelli/Projects/polytext/IMG_20260324_160049070.jpg",
]
DEFAULT_OCR_MODELS = [
    "gemini-2.0-flash",
...
DEFAULT_OUTPUT_DIR = "tests/output/ocr_to_text_comparison"
...
def extract_with_model(
    ...
    start = time.time()
    result = converter.get_ocr(image_file)
...
def compare_quality_with_gemini_pro(
    ...
    prompt = f"""
You are evaluating two OCR outputs extracted from the same image.
...
def run_benchmark(
    ...
    for image_file in image_files:
        logger.info("Processing %s", image_file)
        item = {
        ...
        try:
            for model_name in ocr_models:
                ...
                output_path = run_dir / f"{sanitize_for_filename(image_stem)}.{sanitize_for_filename(model_name)}.md"
            ...
            logger.exception("Failed processing %s", image_file)
            item["error"] = str(exc)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare OCR outputs between two Gemini models and evaluate quality with a third model."
    )
    parser.add_argument(
        "--image-files",
        nargs="+",
        default=DEFAULT_OCR_PATHS,
        help="Local image files to compare",
    )
    parser.add_argument(
        "--models",
        nargs=2,
        default=DEFAULT_OCR_MODELS,
